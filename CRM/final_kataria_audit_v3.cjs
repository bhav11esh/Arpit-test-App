...
const data = JSON.parse(fs.readFileSync('kataria_crm_final_v3.json', 'utf8'));
const userMap = {};
...
function audit(label, file, crm) {
  ...
  dataRows.forEach((r, i) => {
    ...
    // Find match by date and at least one link
    // If sheet has no links, we match by date and photographer
