...
async function compareUsers() {
    ...
    // 2. Users to compare
    const names = ['Sahith', 'Akhil', 'Mallikarjun', 'Aman'];
    ...
    for (const name of names) {
        ...
        if (user.cluster_code === clusterId) {
            console.log(`   Cluster Code: ✅ MATCHES Whitefield`);
        } else if (user.cluster_code) {
            // Find what cluster they are in
            const { data: c } = await supabase.from('clusters').select('name').eq('id', user.cluster_code).single();
            console.log(`   Cluster Code: ❌ MISMATCH (Assigned to ${c ? c.name : user.cluster_code})`);
        } else {
            console.log(`   Cluster Code: ❌ NULL`);
        }
