...
const data = JSON.parse(fs.readFileSync('khivraj_crm_final.json', 'utf8'));
const userMap = {};
...
function audit(label, file, crm) {
  ...
  dataRows.forEach((r, i) => {
    ...
    if (match) {
      ...
      // No issues found with photography mapping so far in other dealerships
    } else {
...
audit('Khivraj Triumph', 'khivraj_triumph_raw.json', data.khivraj);
