...
// Native fetch used
require('dotenv').config({ path: '.env' }); // Only for SERVICE KEY

// Use Service Role to bypass potential RLS issues as seen before
const supabase = createClient(process.env.VITE_SUPABASE_URL, process.env.VITE_SUPABASE_SERVICE_ROLE_KEY);

async function importSkoda() {
    ...
    if (!dealerships || dealerships.length === 0) { console.error('No Skoda dealerships found'); return; }
    ...
    for (const dealership of dealerships) {
        ...
        // Use mapping ID as showroom_code if exists, else dealership ID
        let showroomCode = mappings && mappings.length > 0 ? mappings[0].id : dealership.id;
...
// Add top-level error catching
importSkoda().catch(e => console.error("FATAL SCRIPT ERROR:", e));
