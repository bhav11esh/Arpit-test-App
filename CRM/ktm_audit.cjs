...
const crmData = JSON.parse(fs.readFileSync('ktm_crm_final.json', 'utf8'));
const sheetData = JSON.parse(fs.readFileSync('ktm_popular_raw.json', 'utf8'));
...
dataRows.forEach((r, i) => {
  ...
  const match = crm.find(c => {
    ...
    if (sF || sR) {
      ...
      // Handle edge cases where photographer names might have slight variations
...
if (matchCount === dataRows.length && dataRows.length === crm.length) {
  console.log(`✅ KTM Popular: 100% PARITY!`);
} else {
  console.log(`⚠️ KTM Popular: Issues found. Matches=${matchCount}/${dataRows.length}, CRM=${crm.length}`);
}
