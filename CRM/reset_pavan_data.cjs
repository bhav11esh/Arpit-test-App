...
async function resetPavanData() {
    console.log('--- Resetting data for PAVAN HYUNDAI ---');

    // 1. Find the dealership ID to verify (using ilike for flexibility)
    ...
    if (!dealerships || dealerships.length === 0) {
        console.log('Could not find dealership "Pavan Hyundai".');
        return;
    }
    ...
    if (count > 0) {
        ...
        if (delError) {
            ...
            console.log(`Successfully deleted records for Pavan Hyundai.`);
        }
    } else {
