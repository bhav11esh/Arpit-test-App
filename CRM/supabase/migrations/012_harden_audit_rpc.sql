-- Harden Audit RPC to correctly handle leave statuses and operational date shifts
-- Operational shifts at 4 AM local time (IST)

CREATE OR REPLACE FUNCTION public.get_operational_date(ts TIMESTAMPTZ)
RETURNS DATE AS $$
BEGIN
    -- Operational day shifts at 4 AM local time (IST)
    -- If before 4 AM, it's considered the previous calendar day
    IF EXTRACT(HOUR FROM ts AT TIME ZONE 'Asia/Kolkata') < 4 THEN
        RETURN (ts AT TIME ZONE 'Asia/Kolkata' - INTERVAL '1 day')::DATE;
    END IF;
...
BEGIN
    ...
    -- A. Find Missing "Send Update" (Exclude if day already closed in operational time)
    missing_updates := (
        ...
        WHERE u.role = 'PHOTOGRAPHER' AND u.active = true
          -- Use operational date logic for log events
          AND NOT EXISTS (
              SELECT 1 FROM public.log_events le 
              WHERE le.actor_user_id = u.id 
                AND public.get_operational_date(le.created_at) = target_date 
