...
async function checkFinalization() {
    ...
    console.log(`Checking global finalization for ${showroomCode} on ${today}...`);

    const { data: logs, error } = await supabase
        .from('log_events')
        .select('*')
        .eq('type', 'SHOWROOM_FINALIZED')
        .eq('target_id', showroomCode)
    ...
    if (logs && logs.length > 0) {
        ...
        for (const log of logs) {
            ...
            console.log(`- Finalized by ${user?.name || 'UNKNOWN'} (ID: ${log.user_id}) at ${log.created_at}`);
        }
    } else {
        console.log('No global finalization found.');
    }
}
